  ...
  app.post('/api/short-courses', (req, res) => {
    try {
      ...
      const newCourse = {
        ...
        description: payload.description || '',
        durationWeeks: Number(payload.durationWeeks) || 2,
        durationHours: Number(payload.durationHours) || 20,
        feeGHS: Number(payload.feeGHS) || 0,
        deliveryMode: payload.deliveryMode || 'physical',
        location: payload.location || 'AITI Campus, Sunyani, Ghana',
        upcomingBatches: payload.upcomingBatches || [],
        instructorName: payload.instructorName || 'AITI Certified Faculty',
        instructorTitle: payload.instructorTitle || 'Senior Instructor',
        ...
        prerequisites: payload.prerequisites || 'Basic computer literacy.',
        targetAudience: payload.targetAudience || 'Beginners and professionals looking to upskill.',
